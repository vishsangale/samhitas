"""Thread 17 (docs/threads/17-recall-mechanism-ladder.md), arm (c): DeltaNet-style
outer-product matrix state. Prediction A: does replacing the vector state with a
spectrally-decayed matrix state (delta-rule update) make n_pairs=8 associative recall
solvable? Prediction B (only if A passes): does the trained model's gradient-flow
healthy/unhealthy boundary stay within 2x of ungated orthogonal's closed-form boundary at
eps=0.1?

Protocol matches arms (a)/(b) and threads 9/10/11/16 exactly for direct comparability:
vocab=512, hidden=64, eps=0.1, n_pairs=8 (seq_len=17), 2000 fresh-random-batch Adam steps,
LR grid {3e-4, 1e-3, 3e-3, 1e-2, 3e-2}, 5 seeds.
"""
...
from experiments.models.deltanet_recurrence import DeltaNetRecallModel
from experiments.tasks import recall

RUN_DIR = Path(__file__).resolve().parents[1] / "runs" / "thread17_arm_c_deltanet"
...
CONTROL_ACCS = {"thread9_gated": 0.032, "thread11_dual_gate": 0.032,
                 ...
                 "thread17_arm_b_shortconv": 0.0109, "thread17_arm_a_composition": 0.0227}
...
def train_and_eval(lr, seed):
    ...
    model = DeltaNetRecallModel(VOCAB, HIDDEN, EPS)
